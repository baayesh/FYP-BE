...
@router.post("/messages/conversations/{conversation_id}/participants", response_model=APIResponse)
async def add_participant(
    ...
    try:
        ...
        current_user = _get_user(email, db)
        service = MessageService(db)
        result = service.add_participant(conversation_id, current_user, user_id)
        if not result:
            raise HTTPException(status_code=403, detail="Cannot add participant")
        return APIResponse(success=True, message="Participant added")
    except PermissionError as e:
        raise HTTPException(status_code=403, detail=str(e))
    except HTTPException:
        raise
...
@router.delete("/messages/conversations/{conversation_id}/participants/{user_id}", response_model=APIResponse)
async def remove_participant(
    ...
    try:
        current_user = _get_user(email, db)
        service = MessageService(db)
        result = service.remove_participant(conversation_id, current_user, user_id)
        if not result:
            raise HTTPException(status_code=403, detail="Cannot remove participant")
        return APIResponse(success=True, message="Participant removed")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
